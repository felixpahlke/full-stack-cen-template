...
def test_get_tracking_url_uses_branch_flavor() -> None:
    assert get_tracking_url("local") == "http://localhost/backend-only"
    assert get_tracking_url("production") == "https://backend-only"
...
def test_build_flavor_event_body_marks_backend_source() -> None:
    ...
    assert payload["p"]["app_flavor"] == "backend-only"
    assert payload["u"] == "http://localhost/backend-only"
