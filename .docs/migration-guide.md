...
The pre-upgrade branch tip is tagged `pre-modernization/local-auth-custom-ui`. To inspect or restore it
without overwriting current work:

```bash
git switch --detach pre-modernization/local-auth-custom-ui
```
