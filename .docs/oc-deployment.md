# OpenShift deployment — oauth-proxy-custom-ui
...
When `_DEPLOYMENT_BRANCH_FILTER` is blank, the script resolves it to `oauth-proxy-custom-ui`,
prints it, and verifies that branch through the configured deploy key before creating either
BuildConfig.
...
16. Both BuildConfigs use the printed, remotely verified `oauth-proxy-custom-ui` ref.
