...
You are viewing the `backend-only` flavour: a PostgreSQL-backed API secured with `X-API-Key` and
no bundled frontend.
...
The application screenshots show the full-stack flavours; backend-only flavours do not bundle a
frontend.
