...
```bash
git remote -v
git fetch upstream oauth-proxy
git log --oneline HEAD..upstream/oauth-proxy
git pull --no-rebase --no-commit upstream oauth-proxy
```
...
- Preserve oauth2-proxy, the opaque identity subject, private Basic seam, Carbon UI, environment
