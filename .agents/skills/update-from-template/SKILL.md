...
```bash
git remote -v
git fetch upstream oauth-proxy-custom-ui
git log --oneline HEAD..upstream/oauth-proxy-custom-ui
git pull --no-rebase --no-commit upstream oauth-proxy-custom-ui
```
...
- Preserve oauth2-proxy, the opaque identity subject, private Basic seam, shadcn UI, environment
  names, deployment topology, and application changes. Never reintroduce files the project
