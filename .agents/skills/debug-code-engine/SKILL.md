...
- Wrong public path: only `oauth-proxy` is public; backend/frontend must remain project-only.
  Compare the generated proxy URL and persisted Vite/CORS/OIDC values with application details.
- OAuth 401/redirect failure: inspect `oauth-proxy` logs and secret binding, callback/issuer values,
  project-local upstream names, and the private Basic credential. Do not add a browser bearer
