...
3. Read the dry-run topology and obtain confirmation before the real command. The script confirms
   ...
   registry, secrets, PostgreSQL, backend/frontend BuildConfigs and workloads, Routes, and webhooks.
...
project. The script stages backend/frontend readiness, then exposes oauth2-proxy and removes owned
direct application Routes. Check the proxy URL and `/api/v1/utils/health-check/`; test OIDC sign-in
end to end. Proxy logout does not end the upstream IdP SSO session. For a failure, switch to
`debug-openshift`.
