...
Run `prepare-workstation` first when the checkout is not ready; `pnpm run dev` starts the database.
...
3. `pnpm run db:revision -- -m "<short description>"`.
...
7. Run the affected backend tests and `pnpm run verify`.
